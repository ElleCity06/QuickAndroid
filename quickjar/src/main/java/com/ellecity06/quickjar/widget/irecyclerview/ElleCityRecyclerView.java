...
/**
 * 自定义的RecyclerView自带刷新头部和加载更多
 *
 * @author: ellecity06
 * @e-mail: ellecity06@sina.com
 * @time: 2018/10/17 15:46
 */
public class ElleCityRecyclerView extends RecyclerView {
    ...
    public ElleCityRecyclerView(Context context) {
        this(context, null);
    }

    public ElleCityRecyclerView(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public ElleCityRecyclerView(Context context, AttributeSet attrs, int defStyle) {
    ...
    private void init(Context context) {
        if (pullRefreshEnabled) {
            IRefreshHeader refreshHeader = new IRefreshHeader(context);
            mHeaderViews.put(0, refreshHeader);
            mRefreshHeader = refreshHeader;
        }
        ILoadingMoreFooter footView = new ILoadingMoreFooter(context);
    ...
    public void addHeaderView(View view) {
        if (pullRefreshEnabled && !(mHeaderViews.get(0) instanceof IRefreshHeader)) {
            IRefreshHeader refreshHeader = new IRefreshHeader(getContext());
            mHeaderViews.put(0, refreshHeader);
            mRefreshHeader = refreshHeader;
        }
    ...
    private void loadMoreComplete() {
        ...
        if (previousTotal <= getLayoutManager().getItemCount()) {
            if (footView instanceof ILoadingMoreFooter) {
                ((ILoadingMoreFooter) footView).setState(ILoadingMoreFooter.STATE_COMPLETE);
            } else {
                footView.setVisibility(View.GONE);
            }
        } else {
            if (footView instanceof ILoadingMoreFooter) {
                ((ILoadingMoreFooter) footView).setState(ILoadingMoreFooter.STATE_NOMORE);
            } else {
                footView.setVisibility(View.GONE);
            }
            isnomore = true;
    ...
    public void onScrollStateChanged(int state) {
        ...
        if (state == RecyclerView.SCROLL_STATE_IDLE && mLoadingListener != null && !isLoadingData && loadingMoreEnabled) {
            ...
            if (layoutManager.getChildCount() > 0
                ...
                if (footView != null) {
                    if (footView instanceof ILoadingMoreFooter) {
                        ((ILoadingMoreFooter) footView).setState(ILoadingMoreFooter.STATE_LOADING);
                    } else {
                        footView.setVisibility(View.VISIBLE);
                    }
                }
        ...
        switch (ev.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (isOnTop() && pullRefreshEnabled) {
                    ...
                    if (mRefreshHeader.getVisiableHeight() > 0 && mRefreshHeader.getState() < IRefreshHeader.STATE_REFRESHING) {
                        return false;
                    }
                }
                break;
    ...
    public void setLoadingMoreEnabled(boolean loadingMoreEnabled) {
        ...
        if (!loadingMoreEnabled) {
            ...
            if (mFootViews != null) {
                ILoadingMoreFooter footView = new ILoadingMoreFooter(getContext());
                addFootView(footView, false);
            }
        }
    }
    ...
    public void setLoadMoreGone() {
        ...
        View footView = mFootViews.get(0);
        if (footView != null && footView instanceof ILoadingMoreFooter) {
            mFootViews.remove(0);
        }
    }
    ...
    public void reset() {
        ...
        final View footView = mFootViews.get(0);
        if (footView instanceof ILoadingMoreFooter) {
            ((ILoadingMoreFooter) footView).reSet();
        }
    }
}
