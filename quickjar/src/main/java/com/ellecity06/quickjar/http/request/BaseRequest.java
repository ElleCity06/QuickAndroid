...
import com.ellecity06.quickjar.http.api.ApiService;
...
import com.ellecity06.quickjar.http.interceptor.BaseDynamicInterceptor;
import com.ellecity06.quickjar.http.interceptor.CacheInterceptor;
import com.ellecity06.quickjar.http.interceptor.CacheInterceptorOffline;
import com.ellecity06.quickjar.http.interceptor.HeadersInterceptor;
import com.ellecity06.quickjar.http.interceptor.NoCacheInterceptor;
...
import static com.ellecity06.quickjar.http.ElleCityHttp.getRxCache;

/**
 * 所有请求的基类
 *
 * @author ellecity06
 * @time 2018/7/17 10:13
 * @des
 */
...
public abstract class BaseRequest<R extends BaseRequest> {
    ...
    public BaseRequest(String url) {
        this.url = url;
        context = ElleCityHttp.getContext();
        ElleCityHttp config = ElleCityHttp.getInstance();
    ...
    public R cacheTime(long cacheTime) {
        if (cacheTime <= -1)
            cacheTime = ElleCityHttp.DEFAULT_CACHE_NEVER_EXPIRE;
    ...
    private OkHttpClient.Builder generateOkClient() {
        if (readTimeOut <= 0 && writeTimeOut <= 0 && connectTimeout <= 0 && sslParams == null
            ...
            OkHttpClient.Builder builder = ElleCityHttp.getOkHttpClientBuilder();
            ...
            final OkHttpClient.Builder newClientBuilder = ElleCityHttp.getOkHttpClient().newBuilder();
            if (readTimeOut > 0)
            ...
            if (cookies.size() > 0)
                ElleCityHttp.getCookieJar().addCookies(cookies);
    ...
    private Retrofit.Builder generateRetrofit() {
        if (converterFactories.isEmpty() && adapterFactories.isEmpty()) {
            return ElleCityHttp.getRetrofitBuilder().baseUrl(baseUrl);
        } else {
            ...
            if (!converterFactories.isEmpty()) {
                ...
                //获取全局的对象重新设置
                List<Converter.Factory> listConverterFactory = ElleCityHttp.getRetrofit().converterFactories();
            ...
            if (!adapterFactories.isEmpty()) {
                ...
                //获取全局的对象重新设置
                List<CallAdapter.Factory> listAdapterFactory = ElleCityHttp.getRetrofit().callAdapterFactories();
    ...
    private RxCache.Builder generateRxCache() {
        final RxCache.Builder rxCacheBuilder = ElleCityHttp.getRxCacheBuilder();
        switch (cacheMode) {
            case NO_CACHE://不使用缓存
            ...
            case DEFAULT://使用Okhttp的缓存
                if (this.cache == null) {
                    File cacheDirectory = ElleCityHttp.getCacheDirectory();
                    if (cacheDirectory == null) {
                        cacheDirectory = new File(ElleCityHttp.getContext().getCacheDir(), "okhttp-cache");
                    } else {
                        if (cacheDirectory.isDirectory() && !cacheDirectory.exists()) {
                            cacheDirectory.mkdirs();
                        }
                    }
                    this.cache = new Cache(cacheDirectory, Math.max(5 * 1024 * 1024, ElleCityHttp.getCacheMaxSize()));
                }
                String cacheControlValue = String.format("max-age=%d", Math.max(-1, cacheTime));
                final CacheInterceptor REWRITE_CACHE_CONTROL_INTERCEPTOR = new CacheInterceptor(ElleCityHttp.getContext(), cacheControlValue);
                final CacheInterceptorOffline REWRITE_CACHE_CONTROL_INTERCEPTOR_OFFLINE = new CacheInterceptorOffline(ElleCityHttp.getContext(), cacheControlValue);
