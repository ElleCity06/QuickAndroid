...
package com.ellecity06.quickjar.http;
...
import com.ellecity06.quickjar.http.cache.converter.IDiskConverter;
import com.ellecity06.quickjar.http.cache.converter.SerializableDiskConverter;
import com.ellecity06.quickjar.http.cache.model.CacheMode;
import com.ellecity06.quickjar.http.cookie.CookieManger;
import com.ellecity06.quickjar.http.https.HttpsUtils;
import com.ellecity06.quickjar.http.interceptor.HttpLoggingInterceptor;
import com.ellecity06.quickjar.http.model.HttpHeaders;
import com.ellecity06.quickjar.http.model.HttpParams;
import com.ellecity06.quickjar.http.request.CustomRequest;
import com.ellecity06.quickjar.http.request.DeleteRequest;
import com.ellecity06.quickjar.http.request.DownloadRequest;
import com.ellecity06.quickjar.http.request.GetRequest;
import com.ellecity06.quickjar.http.request.PostRequest;
import com.ellecity06.quickjar.http.request.PutRequest;
 ...
 *
 * @author ellecity06
 * @time 2018/7/17 14:55
 * @des
 */
public final class ElleCityHttp {
    ...
    private volatile static ElleCityHttp singleton = null;

    private ElleCityHttp() {
    ...
    public static ElleCityHttp getInstance() {
        testInitialize();
        if (singleton == null) {
            synchronized (ElleCityHttp.class) {
                if (singleton == null) {
                    singleton = new ElleCityHttp();
                }
            }
        }
        return singleton;
    ...
    private static void testInitialize() {
        if (sContext == null)
            throw new ExceptionInInitializerError("请先在全局Application中调用 ElleCityHttp.init() 初始化！");
    }
     ...
     * 调试模式,默认打开所有的异常调试
     */
    public ElleCityHttp debug(String tag) {
        debug(tag, true);
        return this;
    }
     ...
     */
    public ElleCityHttp debug(String tag, boolean isPrintException) {
        String tempTag = TextUtils.isEmpty(tag) ? "ElleCityHttp_" : tag;
        if (isPrintException) {
     ...
     * https的全局访问规则
     */
    public ElleCityHttp setHostnameVerifier(HostnameVerifier hostnameVerifier) {
     ...
     * https的全局自签名证书
     */
    public ElleCityHttp setCertificates(InputStream... certificates) {
     ...
     * https双向认证证书
     */
    public ElleCityHttp setCertificates(InputStream bksFile, String password, InputStream... certificates) {
     ...
     * 全局cookie存取规则
     */
    public ElleCityHttp setCookieStore(CookieManger cookieManager) {
     ...
     * 全局读取超时时间
     */
    public ElleCityHttp setReadTimeOut(long readTimeOut) {
     ...
     * 全局写入超时时间
     */
    public ElleCityHttp setWriteTimeOut(long writeTimeout) {
     ...
     * 全局连接超时时间
     */
    public ElleCityHttp setConnectTimeout(long connectTimeout) {
     ...
     * 超时重试次数
     */
    public ElleCityHttp setRetryCount(int retryCount) {
        if (retryCount < 0)
     ...
     * 超时重试延迟时间
     */
    public ElleCityHttp setRetryDelay(int retryDelay) {
        if (retryDelay < 0)
     ...
     * 超时重试延迟叠加时间
     */
    public ElleCityHttp setRetryIncreaseDelay(int retryIncreaseDelay) {
     ...
     * 全局的缓存模式
     */
    public ElleCityHttp setCacheMode(CacheMode cacheMode) {
     ...
     * 全局的缓存过期时间
     */
    public ElleCityHttp setCacheTime(long cacheTime) {
        if (cacheTime <= -1)
     ...
     * 全局的缓存大小,默认50M
     */
    public ElleCityHttp setCacheMaxSize(long maxSize) {
     ...
     */
    public ElleCityHttp setCacheVersion(int cacheersion) {
        if (cacheersion < 0)
     ...
     */
    public ElleCityHttp setCacheDirectory(File directory) {
     ...
     * 全局设置缓存的转换器
     */
    public ElleCityHttp setCacheDiskConverter(IDiskConverter converter) {
     ...
     * 全局设置OkHttp的缓存,默认是3天
     */
    public ElleCityHttp setHttpCache(Cache cache) {
        this.mCache = cache;
        return this;
    }
     ...
     * 添加全局公共请求参数
     */
    public ElleCityHttp addCommonParams(HttpParams commonParams) {
     ...
     * 添加全局公共请求参数
     */
    public ElleCityHttp addCommonHeaders(HttpHeaders commonHeaders) {
     ...
     * 添加全局拦截器
     */
    public ElleCityHttp addInterceptor(Interceptor interceptor) {
     ...
     * 添加全局网络拦截器
     */
    public ElleCityHttp addNetworkInterceptor(Interceptor interceptor) {
     ...
     * 全局设置代理
     */
    public ElleCityHttp setOkproxy(Proxy proxy) {
     ...
     * 全局设置请求的连接池
     */
    public ElleCityHttp setOkconnectionPool(ConnectionPool connectionPool) {
     ...
     */
    public ElleCityHttp setOkclient(OkHttpClient client) {
     ...
     */
    public ElleCityHttp addConverterFactory(Converter.Factory factory) {
     ...
     */
    public ElleCityHttp addCallAdapterFactory(CallAdapter.Factory factory) {
     ...
     */
    public ElleCityHttp setCallbackExecutor(Executor executor) {
     ...
     */
    public ElleCityHttp setCallFactory(okhttp3.Call.Factory factory) {
     ...
     * 全局设置baseurl
     */
    public ElleCityHttp setBaseUrl(String baseUrl) {
