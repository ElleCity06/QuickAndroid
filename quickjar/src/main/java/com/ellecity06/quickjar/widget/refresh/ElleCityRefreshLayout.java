 ...
 * @des 智能刷新布局
 */
public class ElleCityRefreshLayout extends ViewGroup {
    private static final String TAG = "ElleCityRefreshLayout";
    ...
    //2、表示开启普通加载
    //3、表示开启预加载模式
    private LoadModel loadMoreModel = LoadModel.NONE;
    //还剩多少个item时触发预加载
    private int advanceCount = 0;

    public ElleCityRefreshLayout(Context context) {
        this(context, null);

    }

    public ElleCityRefreshLayout(Context context, AttributeSet attrs) {
        super(context, attrs);
    ...
    public View getDefaultRefreshView() {
        return new ElleCityRefreshHeader(getContext());
    }
    ...
    private View getDefaultLoadMoreView() {
        return new ElleCityLoadMoreView(getContext());
    }
